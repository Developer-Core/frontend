...
import { SignUpCarpenterCommand } from '../../domain/sign-up-carpenter.command.js';
...
// Closed, invitation-gated carpenter onboarding: the backend validates the
// invitation code, rejects with 403 when it's wrong, and forces the role to
// Carpenter server-side. The code is the gate — no separate role field needed.
const form      = reactive({ firstName: '', lastName: '', email: '', password: '', invitationCode: '' });
...
const invitationCodeError = computed(() => (submitted.value && !form.invitationCode ? t('iam.invitation-code-required') : ''));
const showAuthError = computed(() => submitted.value && store.errors.length > 0);

/**
 * Message shown when registration is rejected: a wrong invitation code (403)
 * gets a specific hint; anything else falls back to the generic failure.
 * @type {import('vue').ComputedRef<string>}
 */
const authErrorMessage = computed(() => {
    const last = store.errors[store.errors.length - 1];
    if (last?.response?.status === 403) return t('iam.invalid-invitation-code');
    return t('iam.register-carpenter-failed');
});

/** Validates the form and, when valid, delegates the carpenter sign-up to the store. */
function performSignUp() {
    submitted.value = true;
    store.errors = [];
    if (firstNameError.value || lastNameError.value || emailError.value || passwordError.value || invitationCodeError.value) return;
    store.signUpCarpenter(new SignUpCarpenterCommand(form), router);
}
</script>
...
<template>
    <div class="auth-screen flex align-items-center justify-content-center p-4">
        <div class="auth-shell w-full">
            ...
            <template #title>{{ t('iam.register-carpenter-title') }}</template>
            <template #subtitle>{{ t('iam.register-carpenter-subtitle') }}</template>
            ...
            <template #content>
                <form class="flex flex-column gap-3 mt-2" novalidate @submit.prevent="performSignUp">
                    ...
                    <div class="field">
                        <label for="invitationCode" class="block mb-1 font-medium">{{ t('iam.invitation-code') }}</label>
                        <pv-icon-field>
                            <pv-input-icon class="pi pi-key" />
                            <pv-input-text id="invitationCode" v-model.trim="form.invitationCode"
                                           class="w-full" :class="{ 'p-invalid': invitationCodeError }"
                                           :placeholder="t('iam.invitation-code-placeholder')" autocomplete="off" />
                        </pv-icon-field>
                        <small v-if="invitationCodeError" class="text-red-500">{{ invitationCodeError }}</small>
                        <small v-else class="text-color-secondary">{{ t('iam.invitation-code-hint') }}</small>
                    </div>
                    ...
                    <pv-button type="submit" :label="t('iam.carpenter-sign-up')" icon="pi pi-user-plus" class="w-full mt-2" />
